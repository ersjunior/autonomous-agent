...
8. **Inbound** — webhooks enfileiram Celery; respostas roteadas ACTIVE/RECEPTIVE; receptivo com fila se capacidade cheia.
9. **Métricas** — campanha/base; seção **Fila de atendimento** (SLA, espera, abandono voz).
10. **Capacidade** — `/dashboard/capacity` (estimativa + Erlang C; planejamento, não runtime).
11. **Tabulações** — `/dashboard/tabulacoes` (catálogo SIP/negócio + customizados).
12. **Devolutiva / monitoramento** — Excel diário (status + tabulação); WebSocket `/api/v1/monitoring/ws`.

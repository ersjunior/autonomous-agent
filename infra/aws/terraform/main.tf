...
locals {
  services = {
    backend  = { port = 8000, image = "${aws_ecr_repository.backend.repository_url}:${var.image_tag}" }
    frontend = { port = 3000, image = "${aws_ecr_repository.frontend.repository_url}:${var.image_tag}" }
    worker   = { port = null, image = "${aws_ecr_repository.worker.repository_url}:${var.image_tag}" }
  }
}
...
resource "aws_ecs_service" "services" {
  for_each = { for k, v in local.services : k => v if k != "worker" }
...
resource "aws_lb_target_group" "services" {
  for_each = { backend = 8000, frontend = 3000 }

...
Esta subseção consolida o **estado atual** do canal de voz, a **motivação** para migrar de turnos com gravação para **Twilio Media Streams** (WebSocket bidirecional), o **roadmap conceitual** (fases 0–4) com o que já foi validado em ambiente real, e o **plano de execução detalhado** (fases A–F). As **Fases A–E** (transporte, VAD, STT por utterance, loop conversacional, hangup autônomo, TTS por turno) estão **✅ IMPLEMENTADAS e validadas ao vivo** (conversa de múltiplos turnos, ~15–20 s por turno). A **Fase F** (barge-in / full-duplex) foi **parcialmente implementada** (F1) mas permanece **desabilitada por padrão** — ver §10.8 abaixo (eco acústico PSTN).

#### Modo stream — estado atual (validado)

Com `VOICE_INBOUND_MODE=stream`, o inbound de voz opera em **turnos otimizados via WebSocket** (sem `<Record>` nem polling Celery):

1. Twilio conecta Media Stream (`<Connect><Stream>`) em `/api/v1/channels/webhooks/voice/media-stream`.
2. **Saudação** inicial (TTS) + frames μ-law outbound.
3. **VAD** (webrtcvad) detecta fim de utterance → STT batch (faster-whisper) → grafo LangGraph → TTS (Coqui 8 kHz) → playback outbound.
4. **Histórico conversacional por ligação** (`chat:{call_sid}` no Redis) — memória longa de conversas anteriores **desligada** na voz (RAG/KB mantido).
5. **Hangup autônomo** (farewell dupla confirmação) + guard de **1º turno** (STT errado em utterance curto não desliga).
6. **Gate anti-eco** no playback (ignora frames inbound durante TTS do agente); padding μ-law reduz ruído ocasional.

**Latência medida ao vivo:** ~**15–20 s** por turno (TTS Coqui batch domina; respostas curtas `voice_response_max_tokens=64` + cap de caracteres reduziram de 30 s+). Caminho futuro: TTS frase-a-frase / streaming de síntese.

**Defaults seguros:** `VOICE_STREAM_BARGE_IN_ENABLED=false`, `VOICE_STREAM_ECHO_DEBUG_CAPTURE=false` (instrumentação de diagnóstico de eco, ver `echo_capture.py`).

**Limitações conhecidas:**

- **STT (Whisper):** utterances muito curtos podem ser transcritos errado (ex.: "Olá" → "Tchau") — mitigado pelo guard de 1º turno no farewell (não desliga sem histórico).
- **Barge-in (F1):** implementado (detecção de fala sustentada, mensagem `clear` Twilio, aborto de envio) mas **desabilitado** — o track inbound PSTN contém **eco acústico** do próprio agente, causando falso barge-in. Solução exigiria **AEC** (cancelamento de eco acústico), subprojeto de processamento de sinal. Material de defesa: limitação identificada, medida e documentada.
- **Ruído ocasional** em frames μ-law — mitigado com padding; investigação contínua.
...
**Status do plano de execução:** as **Fases A–E** estão **✅ IMPLEMENTADAS e validadas ao vivo** (modo `stream`). A **Fase F** (barge-in) tem **F1 implementado** mas **desligado por padrão** (eco acústico — trabalho futuro / AEC). A Fase **0 conceitual** (PoC transporte) está **✅ VALIDADA**.
...
**Status:** ✅ **Implementada** — `VOICE_INBOUND_MODE=stream` ativa `<Connect><Stream>` + WS `/webhooks/voice/media-stream`; saudação TTS; pipeline completo A–E.
...
Gap atual: o projeto **possui VAD** no ramo stream (`agents/channels/voice/audio_pipeline.py`); fim de fala no modo `record` continua sendo o timeout do `<Record>` da Twilio.

**Status Fase B:** ✅ **Implementada** (VAD + buffer de utterance + fila FIFO).
...
**Status Fase C:** ✅ **Implementada** (STT por utterance fechada pelo VAD).

##### Fase D — Agente + TTS por turno: loop completo (marco)
...
**Status Fase D:** ✅ **Implementada** (agente + TTS Coqui 8 kHz por turno; latência ~15–20 s; TTS frase-a-frase pendente).
...
**Status Fase E:** ✅ **Implementada** (hangup autônomo, guard 1º turno farewell, histórico por `call_sid`, identidade ancorada, suite de testes stream).

##### Fase F — (opcional, futura) Barge-in + full-duplex

| | |
|---|---|
| **Objetivo** | **Barge-in**: lead interrompe o agente enquanto fala (full-duplex). Estender `ProviderFactory` para STT/TTS com streaming nativo via API comercial quando necessário. |
| **Estado F1 (implementado, desligado)** | Detecção de fala sustentada no inbound durante playback; mensagem `clear` da Twilio; aborto do envio outbound. Flag: `VOICE_STREAM_BARGE_IN_ENABLED` (**default `false`**). |
| **Bloqueio técnico** | **Eco acústico PSTN:** o track inbound contém a voz do próprio agente; o VAD interpreta como fala do lead → **falso barge-in** (agente interrompe a si mesmo). Solução: **AEC** (Acoustic Echo Cancellation) — subprojeto de DSP, não trivial em telefonia. Instrumentação: `agents/channels/voice/echo_capture.py` + `VOICE_STREAM_ECHO_DEBUG_CAPTURE`. |
| **Entrega testável** | Barge-in demonstrável **após** AEC ou ambiente sem eco (ex.: fones). Provedor comercial plugável por env. |
| **Risco** | **Alto** para PSTN sem AEC; **médio** com APIs streaming nativas. |
| **Modo record** | Intacto — barge-in só existe no ramo `stream`. |

Se o stack local travar em TTS frase-a-frase, o fallback natural é **API com streaming nativo** e/ou **manter `record` em produção**.
...
| **Full-duplex / barge-in** | F | F1 implementado; **desligado por padrão** — eco acústico PSTN exige AEC antes de produção. |
...
| **Decisão atual** | **Mantido** — `VOICE_INBOUND_MODE=record` fallback | **✅ Validado** — `VOICE_INBOUND_MODE=stream` (Fases A–E) | **F1 implementado, off** — barge-in aguarda AEC |
